...
- `world.js` — a 1.3km procedural city. Nothing is baked into one giant ground texture: at city
  ...
  geometry, and every road marking is a painted quad sitting on the surface.

  Every surface also gets a **normal map and a roughness map derived from its own albedo**,
  treating luminance as a height field and running a wrapped Sobel over it. That is what makes
  aggregate, slab joints and window reveals catch the sun instead of looking like printed paper.
  It costs about 60ms per map at load.

  A daytime sky is drawn to an equirectangular canvas and run through `PMREMGenerator`, so it
  lights the scene and glass genuinely reflects it; the background slowly drifts. Buildings carry
  a storefront band at street level — glazing with lit interiors, stall risers, awnings, muted
  fascia signage — sampled at a random U offset per building so neighbours never line up. Three
  rings of silhouette blocks sit past the playable edge so the city doesn't visibly stop.
  Static geometry is merged per material **and per district chunk**, so both the camera and the
  shadow pass frustum-cull whole neighbourhoods.
- **Traffic signals** stand at every junction on mast arms. Because the whole city shows one
  aspect per axis, all the lenses merge into six meshes and the entire network animates by
  touching six materials. The phase is driven off the loop clock, so the lights repeat exactly
  every loop — a junction you caught on green is green again for your echo. Traffic brakes as
  hard as it needs to in order to stop *on* the line, rather than stamping on it early.
- `actors.js` — arcade car physics (longitudinal/lateral velocity split, so you can drift on the
  handbrake), with **weight transfer**: the body dives under braking and squats under power, and
  rolls into corners. Brake lights and reverse lights are driven from the actual velocity, not
  the input. Cars are built by tapering box volumes into a real silhouette — raked greenhouse,
  bonnet and boot decks, arches, bumpers, separate tyres and rims. People are capsule-built with
  swinging feet. Traffic keeps to the correct lane rather than straddling the centre line,
  pedestrians flinch out of the way, and police bias onto the street grid when pursuing from far off.
- `game.js` — the loop machinery. Player state is recorded at 20 Hz into a ring of frames; each
  ...
  clock. Relays, the vault, the wanted level, arrest meter, minimap and timeline all live here, plus
  pooled **skid marks** that stretch to bridge whatever distance was covered since the last frame,
  so the trail is continuous at any frame rate — and gets wiped when the city snaps back.
- `audio.js` — Web Audio synthesis. The engine is three detuned oscillators through a moving
  lowpass driven by a **simulated gearbox** — revs climb through a ratio, drop on the shift and
  climb again, rather than one speed-proportional whine; sirens, tyre scrub, crashes and the rewind sweep are all generated.
